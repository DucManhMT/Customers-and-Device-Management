...
public class TechEmployeeController extends HttpServlet {
    ...
    private TechEmployeeService techEmployeeService;
    
    @Override
    public void init() throws ServletException {
        super.init();
        techEmployeeService = new TechEmployeeService();
    }
    ...
    private void showTechEmployeesList(HttpServletRequest req, HttpServletResponse resp) 
        ...
        try {
            ...
            if (hasFilters) {
                techEmployeePage = techEmployeeService.getTechEmployeesWithFilters(
                    searchName, location, ageRange, page, recordsPerPage);
                totalCount = (int) techEmployeePage.getTotalElements();
            } else {
                techEmployeePage = techEmployeeService.getTechEmployeesPaginated(page, recordsPerPage);
                totalCount = techEmployeeService.getTechEmployeeCount();
            }
            
            List<String> availableLocations = techEmployeeService.getAvailableLocations();
    ...
    private void showTechEmployeeDetail(HttpServletRequest req, HttpServletResponse resp) 
        ...
        try {
            ...
            Staff techEmployee = techEmployeeService.getTechEmployeeById(staffId);
