...
public class TechEmployeeDAO extends SimpleRepository<Staff, Integer> {
    ...
    public Page<Staff> findTechEmployeesWithFilters(int roleId, String searchName, String location, 
        ...
        queryBuilder.append("SELECT s.StaffID, s.StaffName, s.Phone, s.Address, s.Email, s.Image, s.DateOfBirth, s.Username ");
    ...
    private String getAgeCondition(String ageRange) {
        ...
        switch (ageRange.toLowerCase()) {
            case "18-25":
                return baseCondition + " AND (YEAR(CURDATE()) - YEAR(s.DateOfBirth) - (DATE_FORMAT(CURDATE(), '%m%d') < DATE_FORMAT(s.DateOfBirth, '%m%d'))) BETWEEN 18 AND 25";
            case "26-35":
                return baseCondition + " AND (YEAR(CURDATE()) - YEAR(s.DateOfBirth) - (DATE_FORMAT(CURDATE(), '%m%d') < DATE_FORMAT(s.DateOfBirth, '%m%d'))) BETWEEN 26 AND 35";
            case "36-45":
                return baseCondition + " AND (YEAR(CURDATE()) - YEAR(s.DateOfBirth) - (DATE_FORMAT(CURDATE(), '%m%d') < DATE_FORMAT(s.DateOfBirth, '%m%d'))) BETWEEN 36 AND 45";
            case "46-55":
                return baseCondition + " AND (YEAR(CURDATE()) - YEAR(s.DateOfBirth) - (DATE_FORMAT(CURDATE(), '%m%d') < DATE_FORMAT(s.DateOfBirth, '%m%d'))) BETWEEN 46 AND 55";
            case "56+":
                return baseCondition + " AND (YEAR(CURDATE()) - YEAR(s.DateOfBirth) - (DATE_FORMAT(CURDATE(), '%m%d') < DATE_FORMAT(s.DateOfBirth, '%m%d'))) >= 56";
            default:
                return null;
        }
    }
